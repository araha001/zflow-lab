    ...
    <div className="grid gap-4 p-6">
      <Card>
        <CardContent className="grid gap-2">
          <h2 className="text-xl font-bold">Zflow 정렬 진단기</h2>
          <Input
            placeholder="여기에 정렬 문장을 입력해보세요"
            value={input}
            onChange={(e) => setInput(e.target.value)}
          />
          <Button onClick={analyzeInput}>정렬 분석하기</Button>
        </CardContent>
      </Card>
      ...
      {level && (
        <Card>
          <CardContent className="grid gap-2">
            ...
            <p>정렬 점수: {score}점</p>
            <p>등급: {level}</p>
            <p className="text-sm text-muted-foreground">미션: {mission}</p>
          </CardContent>
        </Card>
      )}
      ...
      {graphData.length > 0 && (
        <Card>
          <CardContent>
            <h3 className="text-lg font-semibold">정렬 요소 분석</h3>
